...
SRC_PREFIX = "master/city."
DST_PREFIX = "CDP/city/"   # đẩy thẳng vào folder cố định, KHÔNG có yyyymm
...
def copy_idempotent_no_dup(src_bucket, src_obj, dst_bucket, dst_obj):
    ...
    if hook.exists(dst_bucket, dst_obj):
        return "skipped_exists"  # không overwrite để tránh duplicate
    hook.copy(src_bucket, src_obj, dst_bucket, dst_obj)
    return "copied"

with DAG(
    dag_id="dag_city_success",
    start_date=datetime(2024, 1, 1),
    schedule=None, catchup=False,
    tags=["city", "success", "gcs"],
) as dag:
